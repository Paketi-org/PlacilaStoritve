...
h = handler.FluentHandler(
    "Placila", host=app.config["FLUENT_IP"], port=int(app.config["FLUENT_PORT"])
)
formatter = handler.FluentRecordFormatter(custom_format)
h.setFormatter(formatter)
l.addHandler(h)
l.info(
    "Pripravljanje Placila Mikrostoritve",
    extra={
        "name_of_service": "Placila",
        "crud_method": None,
        "directions": None,
        "ip_node": None,
        "status": None,
...
api = Api(
    ...
    doc="/placila/openapi",
    title="Placila API",
    description="Abstrakt Placila API",
    default_swagger_filename="openapi.json",
    default="Placila CRUD",
    default_label="koncne tocke in operacije",
)
placiloApiModel = api.model(
    "ModelPlacila",
    {
        ...
        "id_prejemnika": fields.Integer(
            readonly=True, description="ID prejemnika placila"
        ),
        "znesek_eur": fields.String(readonly=True, description="Znesek placila v EUR"),
        "znesek_coin": fields.String(
            readonly=True, description="Znesek placila v bitcoin"
        ),
        "status": fields.String(readonly=True, description="Status placila"),
    },
)
placilaApiModel = api.model(
    "ModelPlacil", {"placila": fields.List(fields.Nested(placiloApiModel))}
)
ns = api.namespace("Placila CRUD", description="Placila koncne tocke in operacije")
posodobiModel = api.model(
    "PosodobiPlacilo", {"atribut": fields.String, "vrednost": fields.String}
)
metrics = PrometheusMetrics(app)

grpc_channel = grpc.insecure_channel(
    "{}:{}".format(app.config["GRPC_SERVER_IP"], app.config["GRPC_SERVER_PORT"])
)
...
class Placilo(Resource):
    ...
    def get(self, id):
        ...
        """
        l.info(
            "Zahtevaj placilo z ID %s" % str(id),
            extra={
                "name_of_service": "Placila",
                "crud_method": "get",
                "directions": "in",
        ...
        if len(row) == 0:
            l.warning(
                "Placilo z ID %s ni bil najden" % str(id),
                extra={
                    "name_of_service": "Placila",
                    "crud_method": "get",
                    "directions": "out",
    ...
    def put(self, id):
        ...
        """
        l.info(
            "Posodobi placilo z ID %s" % str(id),
            extra={
                "name_of_service": "Placila",
                "crud_method": "put",
                "directions": "in",
        ...
        l.info(
            "Placilo z ID %s posodobljeno" % str(id),
            extra={
                "name_of_service": "Placila",
                "crud_method": "put",
                "directions": "out",
...
class ListPlacil(Resource):
    def __init__(self, *args, **kwargs):
        ...
        self.parser.add_argument(
            "id", type=int, required=True, help="ID plačila je obvezen"
        )
        self.parser.add_argument(
            "id_placnika", type=int, required=True, help="ID plačnika je obvezen"
        )
        self.parser.add_argument(
            "id_prejemnika", type=int, required=True, help="ID prejemnika je obvezen"
        )
        self.parser.add_argument(
            "znesek_eur",
            type=str,
            required=True,
            help="Znesek plačila v EUR je obvezen",
        )
        self.parser.add_argument(
            "status", type=str, required=True, help="Status plačila je obvezen"
        )
    ...
    def post(self):
        ...
        self.cur.execute(
            """INSERT INTO {0} (id, id_placnika, id_prejemnika, znesek_eur, znesek_coin, status)
